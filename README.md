...
Pieces are saved as `.gtab` — the small, git-diffable [text format](docs/TEXTFORMAT.md) — into the app's own pieces folder by default, where they show up in your library. **ctrl+S** asks for a name right in the window and saves there — no OS dialog needed (**ctrl+shift+S** opens one for saving somewhere else). You **hear each note as you place it**, played on the track's own voice, and **shift+P** saves and opens the piece for practice; **esc** comes back to the editing, so writing a bar, hearing it, and fixing it is a loop rather than a round trip. A first run also seeds the library with two tiny demo pieces — one guitar, one soprano sax — so there is something to play before you have written a note.
