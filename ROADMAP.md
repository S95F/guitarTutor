...
- [ ] **Real-recording validation** — everything above is synthesis. A mic'd sax brings room noise, breath, key clicks, and playback bleed (see Known Risks). The corpus harness is ready for it — a wind category with a filename-is-the-expectation contract (`internal/pitch`, layout in [docs/TESTDATA.md](docs/TESTDATA.md), skipped while empty) — but no recordings exist yet, so every threshold stays provisional until they do
- [x] Wind parts in the notation editor: a fixed pitch-ladder grid (height = written pitch, octave C lines labelled, the reading the practice view teaches), note-letter entry with semitone/octave arrows, the wind mark set (slur, slide, bend, vibrato) on the toolbar — and an instrument picker on every new piece and new track, so a horn part starts on the grid instead of in the text view
- [x] Guitar Pro wind parts: clean-room `ConcertPitch`/`TransposedPitch` reading, instrument resolved by declared GM program or by the written-minus-concert transposition plus range (never by name), any authored tuning or fingering vetoing the wind path; everything ambiguous keeps today's skip-with-warnings behavior, and the warnings stay the evidence trail for what real files carry
- [x] MusicXML slur → `TechSlur` mapping (imported wind lines arrive slurred as written, not tongued)
- [x] Soft same-pitch tonguing: a dip-recovery onset path for wind detector configs — a hop dipping ≥ 5 dB below the smoothed level then recovering within ~8 hops is a re-articulation, catching the soft strokes whose spectral flux never fires — tuned against the reed voice and provisional until the real-recording corpus lands
